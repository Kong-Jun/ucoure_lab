 ...
 * vpd is set bellow.
 * */
/* vpt指向第第一个页表的第一个词条, vpt[n]代表第一个页表的第n个词条
 * 肯定不会产生page fault */
pte_t * const vpt = (pte_t *)VPT;
/* vpt是页目录的虚拟地址,通过它可以直接访问页目录 */
...
// #if 0
	...
	/* if PTE not exists, allocated one page PTE point to and create PTE */
	// if (!test_bit(PTE_P, vpt[ptx])) {
	// 	struct_page_vp = alloc_page();
	// 	assert(struct_page_vp != NULL);
	// 	page_pa = page2pa(struct_page_vp);
	// 	vpt[vpx] = page_pa | PTE_P | PTE_U | PTE_W;	
	// }
